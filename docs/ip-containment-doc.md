...
```mermaid
    flowchart TD
        Start --> user_input --> check1 --> convert --> call_collect --> create_sets --> loop1 --> populate_internal --> loop2 --> check_relation --> return_collect --> return_check1 --> End
        ...
        check1[The code calls the group's __contains__ method, passing the IP address to the method and a reference to group as the current group.]
        ...
        modify_included[Modify included...]
        modify_excluded[Modify excluded...]
        return_collect[Return the CidrSet created from the set difference between the set of the union of internal and included and the excluded set.]
